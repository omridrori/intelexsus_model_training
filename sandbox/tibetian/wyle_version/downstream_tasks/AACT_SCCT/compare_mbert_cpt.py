"""Compare baseline mBERT vs CPT-continued mBERT on tib_03_task Wylie (task_3_label).
Trains each model for a few epochs, evaluates every 20 steps, and plots F1 curves.
"""
...
RESULTS_DIR = HERE / 'results' / 'compare_mbert_cpt_results'
...
def train_and_collect(tag: str, model_path: str, tok_path: str, ds: DatasetDict, labels: List[str], args):
    ...
    mdl = AutoModelForSequenceClassification.from_pretrained(model_path, num_labels=len(labels),
    ...
    trainer = Trainer(model=mdl, args=tr_args, train_dataset=ds_tok['train'], eval_dataset=ds_tok['test'],
                      tokenizer=tok, data_collator=collator, compute_metrics=compute_f1, callbacks=[LogF1Callback()])
    trainer.train()
...
def plot_curves(curves: Dict[str, List[tuple[int, float]]]):
    ...
    plt.title('mBERT vs CPT Fine-Tuning (task3)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
