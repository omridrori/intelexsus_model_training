...
Productionised version of `sandbox/sanskrit/cpt_bert/train_bert_continual.py`.
...
import os
# Ensure the env var is set before importing libs that initialise OpenMP
...
def _load_custom_tokenizer(tokenizer_path: Path | str) -> BertTokenizerFast:
    ...
    if (tokenizer_path / "vocab.txt").exists():
        ...
        try:
            ...
            # Fallback to direct constructor if repository-style files are missing
...
def continual_pretrain_mbert(
    ...
    Parameters
    ----------
    data_file: path to training text (defaults to `sanscrit.config.BERT_READY_FILE`).
    model_output_dir: destination for checkpoints (defaults to `sanscrit.config.CONTINUAL_OUTPUT_DIR`).
    training_args_kwargs: overrides for `transformers.TrainingArguments`.
    num_epochs: optional override for epoch count.
    base_model: HF model id or local path to start from.
    """
    # Paths
    data_file = Path(data_file or BERT_READY_FILE)
    model_output_dir = Path(model_output_dir or CONTINUAL_OUTPUT_DIR)
